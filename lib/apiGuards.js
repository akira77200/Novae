// lib/apiGuards.js — Protections communes pour les APIs
...
export async function requireAuth(req) {
  ...
  if (!authHeader || !authHeader.startsWith('Bearer ')) {
    console.error('[requireAuth] Missing or invalid auth header')
    return { ok: false, user: null, error: 'Connexion requise.' }
  }

  const token = authHeader.replace('Bearer ', '').trim()
  if (!token || token === 'undefined' || token === 'null' || token === '') {
    console.error('[requireAuth] Invalid token value:', token)
  ...
  console.log('[requireAuth] Auth success for user:', user.email)
