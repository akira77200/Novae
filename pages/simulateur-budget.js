...
export default function SimulateurBudget() {
  ...
  const jauge = (besoin) => besoin > 40000 ? { color:'#DC2626', label: lang==='fr'?'Budget serré':'Tight budget', bg:'rgba(248,113,113,0.08)' }
    : besoin > 25000 ? { color:'var(--text-muted)', label: lang==='fr'?'Budget modéré':'Moderate budget', bg:'rgba(251,191,36,0.08)' }
    : { color:'var(--text-body)', label: lang==='fr'?'Budget gérable':'Manageable budget', bg:'rgba(52,211,153,0.08)' }
  ...
  const Field = ({ label, value, setter, options }) => (
    <div style={{ marginBottom: 14 }}>
      <label style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.6, display: 'block', marginBottom: 6 }}>{label}</label>
  ...
  const BudgetCard = ({ budget, label, color }) => {
    ...
    const postes = [
      { label: lang==='fr'?'Frais de scolarité (moy.)':'Tuition (avg.)', val: budget.frais_scolarite, color: 'var(--text-muted)' },
      { label: lang==='fr'?'Loyer (12 mois)':'Rent (12 months)',          val: budget.loyer,           color: 'var(--text-h1)' },
      { label: lang==='fr'?'Épicerie + transport':'Groceries + transit',   val: budget.epicerie_transport, color: 'var(--text-h1)' },
      { label: lang==='fr'?'Téléphone + internet':'Phone + internet',      val: budget.telephone_internet, color: 'var(--text-muted)' },
      { label: lang==='fr'?'Loisirs':'Leisure',                            val: budget.loisirs,         color: 'var(--text-faint)' },
      { label: lang==='fr'?'Divers + imprévus (10%)':'Misc + unexpected (10%)', val: budget.divers,    color: 'var(--text-faint)' },
    ]

    return (
      <div style={{ background: 'var(--bg-card)', border: `1px solid ${color}30`, borderRadius: 16, overflow: 'hidden', flex: 1, minWidth: 280 }}>
        <div style={{ height: 4, background: color }} />
        <div style={{ padding: '18px 20px' }}>
          <p style={{ fontSize: 15, fontWeight: 800, color: 'var(--text-h1)', marginBottom: 18 }}>{label}</p>
              ...
              return (
                <div key={i}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-h1)' }}>{p.val.toLocaleString()} $</span>
                  </div>
          ...
          <div style={{ borderTop: '1px dashed var(--border)', paddingTop: 14, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <span style={{ fontSize: 13, color: 'var(--text-muted)' }}>{lang==='fr'?'Total dépenses':'Total expenses'}</span>
              <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-h1)' }}>{budget.total_depenses.toLocaleString()} $</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
              <span style={{ fontSize: 13, color: 'var(--text-body)' }}>- {lang==='fr'?'Revenus travail':'Work income'}</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-body)' }}>- {budget.revenu_travail.toLocaleString()} $</span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 14 }}>
              <span style={{ fontSize: 13, color: 'var(--text-body)' }}>- {lang==='fr'?'Bourse estimée':'Est. scholarship'}</span>
              <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-body)' }}>- {budget.bourse_estimee.toLocaleString()} $</span>
            </div>
          </div>
      ...
      <main style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap', alignItems: 'flex-start' }}>
          ...
          <div style={{ width: '100%', maxWidth: 260, flexShrink: 0 }}>
            ...
            {/* Scénario A */}
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px', marginBottom: 14 }}>
              <p style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-body)', marginBottom: 14 }}>
            ...
            {!compareMode ? (
              <button onClick={() => setCompareMode(true)}
                style={{ width: '100%', padding: '11px', background: 'transparent', border: '1px dashed var(--border)', borderRadius: 12, color: 'var(--text-muted)', fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
              ...
              <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '18px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 14 }}>
                  ...
                  <button onClick={() => setCompareMode(false)}
                    style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: 12, cursor: 'pointer' }}>✕</button>
                </div>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            {compareMode && budgetA && budgetB && (
              <div style={{ marginTop: 14, padding: '14px 18px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12 }}>
                {(() => {
            ...
            {budgetA && (() => {
              ...
              return lines && lines.length > 0 && (
                <div style={{ marginTop: 14, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px' }}>
                  <p style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 12 }}>
                    {lang==='fr'?'Conseils personnalisés':'Personalized tips'}
                  </p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                    {lines.map((l, i) => (
                      <p key={i} style={{ fontSize: 13, color: 'var(--text-h1)', lineHeight: 1.6, padding: '8px 12px', background: 'var(--bg-subtle)', borderRadius: 8 }}>
                        {l}
                      </p>
                    ))}
                  </div>
                ...
                <div style={{ marginTop: 14, background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 14, padding: '16px 18px' }}>
                  ...
                  {scenarios.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {scenarios.map(s => (
                        <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', background: 'var(--bg-subtle)', borderRadius: 10, gap: 10, flexWrap: 'wrap' }}>
                          <div>
                            <p style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-h1)' }}>
                              {s.ville} · {s.type === 'universite' ? (lang === 'fr' ? 'Université' : 'University') : (lang === 'fr' ? 'Collège' : 'College')}
                            </p>
                            <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{s.date} · {lang === 'fr' ? 'Besoin' : 'Need'}: <strong style={{ color: 'var(--text-body)' }}>{s.besoin.toLocaleString()} $</strong></p>
                          </div>
            ...
            <div style={{ marginTop: 14, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              <a href="/bourses" style={{ padding: '8px 16px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 9, color: 'var(--text-body)', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
                🎓 {lang==='fr'?'Trouver des bourses →':'Find scholarships →'}
              </a>
              <a href="/orientation-type" style={{ padding: '8px 16px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 9, color: 'var(--text-body)', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
                🏛️ {lang==='fr'?'Univ. ou Collège →':'Univ. or College →'}
              </a>
              <a href="/cv" style={{ padding: '8px 16px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 9, color: 'var(--text-body)', fontSize: 12, fontWeight: 600, textDecoration: 'none' }}>
