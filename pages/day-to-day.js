...
const POSTES_BUDGET = [
  { id: 'loyer',     fr: 'Loyer',      en: 'Rent',      icon: '🏠', color: 'var(--text-h1)' },
  { id: 'epicerie',  fr: 'Épicerie',   en: 'Groceries', icon: '🛒', color: 'var(--text-muted)' },
  { id: 'transport', fr: 'Transport',  en: 'Transit',   icon: '🚇', color: 'var(--text-muted)' },
  { id: 'telephone', fr: 'Téléphone',  en: 'Phone',     icon: '📱', color: 'var(--text-faint)' },
  { id: 'internet',  fr: 'Internet',   en: 'Internet',  icon: '📡', color: 'var(--text-muted)' },
  { id: 'loisirs',   fr: 'Loisirs',    en: 'Leisure',   icon: '🎭', color: 'var(--text-h1)' },
]
...
const CATEGORIES = [
  { id: 'logement',   fr: 'Logement',            en: 'Housing',            icon: '🏠', color: 'var(--text-h1)', query: null },
  { id: 'budget',     fr: 'Budget mensuel',      en: 'Monthly budget',     icon: '📊', color: 'var(--text-body)', query: null },
  { id: 'convertir',  fr: 'Convertisseur',       en: 'Converter',          icon: '💱', color: 'var(--text-muted)', query: null },
  { id: 'halal',      fr: 'Épiceries halal',     en: 'Halal groceries',    icon: '🥩', color: 'var(--text-h1)', query: (c) => `halal+grocery+${c}` },
  { id: 'hair',       fr: 'Coiffeurs afro',      en: 'Afro hairdressers',  icon: '💈', color: 'var(--text-faint)', query: (c) => `afro+hair+salon+${c}` },
  { id: 'exotic',     fr: 'Épiceries exotiques', en: 'Exotic stores',      icon: '🛒', color: 'var(--text-muted)', query: (c) => `african+caribbean+grocery+${c}` },
  { id: 'prices',     fr: 'Prix comparés',       en: 'Price comparison',   icon: '💰', color: 'var(--text-muted)', query: null },
]
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)', color: 'var(--text-h1)', fontFamily: 'system-ui,sans-serif' }}>
      ...
      <main style={{ maxWidth: 820, margin: '0 auto', padding: '36px 20px 80px' }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--text-muted)', marginBottom: studyCitySuggestion ? 16 : 28, lineHeight: 1.6 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
          <span style={{ fontSize: 13, color: 'var(--text-muted)', fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.7, flexShrink: 0 }}>
            {lang === 'fr' ? 'Ville' : 'City'}
          </span>
          <input value={city} onChange={e => setCity(e.target.value)}
            placeholder="Montreal, Toronto..."
            style={{ maxWidth: 240, padding: '9px 13px', background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 9, color: 'var(--text-h1)', fontSize: 14, outline: 'none', colorScheme: 'dark' }} />
        </div>
        ...
        {active === 'prices' && (
          <div style={{
            ...
            <p style={{ color: 'var(--text-h1)', fontSize: '0.85rem', fontWeight: 600 }}>
              {lang === 'fr'
          ...
          <div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
              {POSTES_BUDGET.map(p => {
                ...
                return (
                  <div key={p.id} style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 12, padding: '13px 16px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8 }}>
                      <span style={{ fontSize: 18, flexShrink: 0 }}>{p.icon}</span>
                      <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-h1)', flex: 1 }}>{lang === 'fr' ? p.fr : p.en}</span>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                        <input
                          type="number" min="0" step="10"
                          value={val}
                          onChange={e => setVal(p.id, e.target.value)}
                          style={{ width: 80, padding: '5px 8px', background: 'var(--bg-subtle)', border: '1px solid var(--border)', borderRadius: 7, color: 'var(--text-h1)', fontSize: 13, textAlign: 'right', outline: 'none' }}
                        />
            ...
            <div style={{ padding: '18px 22px', background: 'var(--bg-subtle)', border: '1px solid var(--border)', borderRadius: 14, marginBottom: 16 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
                <div>
                  ...
                  <p style={{ fontSize: 32, fontWeight: 800, color: 'var(--text-body)', letterSpacing: -1 }}>{total.toFixed(0)} <span style={{ fontSize: 16, fontWeight: 500 }}>$ CAD</span></p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontSize: 12, color: 'var(--text-muted)', marginBottom: 4 }}>
                    {lang === 'fr' ? `Salaire min. ${villeData.smig_h.toFixed(2)} $/h` : `Min. wage ${villeData.smig_h.toFixed(2)} $/h`}
                  </p>
                  <p style={{ fontSize: 20, fontWeight: 700, color: 'var(--text-muted)' }}>
                    ~{hMois}h <span style={{ fontSize: 13, fontWeight: 400, color: 'var(--text-muted)' }}>{lang === 'fr' ? '/ mois pour couvrir' : '/ month to cover'}</span>
                  </p>
                </div>
              </div>
            </div>
          ...
          return (
            <div>
              ...
              <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16, padding:'20px', marginBottom:20 }}>
                <p style={{ fontSize:13, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:0.6, marginBottom:14 }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between' }}>
                  <span style={{ fontSize:11, color:'var(--text-muted)' }}>400 $</span>
                  <span style={{ fontSize:11, color:'var(--text-muted)' }}>2 500 $</span>
                </div>
              </div>

              {/* SECTION 2 — Options selon budget */}
              <p style={{ fontSize:13, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:0.6, marginBottom:12 }}>
                {lang==='fr'
              ...
              {optionsDispo.length === 0 ? (
                <div style={{ padding:'24px', background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, textAlign:'center', marginBottom:20 }}>
                  <p style={{ fontSize:36, marginBottom:10 }}>💸</p>
                  <p style={{ fontSize:14, color:'var(--text-h1)', fontWeight:600, marginBottom:6 }}>
                    ...
                    <div key={i} style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:14, overflow:'hidden' }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 18px', borderBottom:'1px solid var(--border)' }}>
                        ...
                        <div style={{ flex:1 }}>
                          ...
                          <p style={{ fontSize:12, color:'var(--text-muted)' }}>{lang==='fr'?opt.description:opt.description_en}</p>
                        </div>
                        <div style={{ textAlign:'right', flexShrink:0 }}>
                          <p style={{ fontSize:16, fontWeight:800, color:'var(--text-h1)' }}>{opt.budget_min}–{opt.budget_max} $</p>
                          <p style={{ fontSize:10, color:'var(--text-muted)' }}>{lang==='fr'?'/ mois':'/ month'}</p>
                        </div>
                      </div>
              ...
              <div style={{ background:'var(--bg-card)', border:'1px solid var(--border)', borderRadius:16, padding:'20px' }}>
                <p style={{ fontSize:14, fontWeight:700, color:'var(--text-h1)', marginBottom:14 }}>
        ...
        {active === 'convertir' && (
          <div>
                ...
                {DEVISES.map(d => (
                  <button key={d.code} onClick={() => { setDeviseCode(d.code); setMontant(String(d.ex)) }}
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{d.pays}</p>
                    </div>
                    <span style={{ fontSize: 12, color: 'var(--text-muted)' }}>1 CAD = {d.taux} {d.code}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            {/* Zone de conversion */}
            <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, padding: '22px', marginBottom: 20 }}>
            ...
            {/* Raccourcis montants */}
            <div>
              <p style={{ fontSize: 11, fontWeight: 600, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 }}>
            ...
            <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 18, fontStyle: 'italic', lineHeight: 1.6 }}>
              ⚠ {lang === 'fr'
