...
const STATUT_CFG = {
  valide:     { label: 'Valide',       labelEn: 'Valid',      color: '#3A3D40', bg: 'rgba(52,211,153,0.12)',  icon: '✅' },
  expire:     { label: 'Expiré',       labelEn: 'Expired',    color: '#DC2626', bg: 'rgba(248,113,113,0.12)', icon: '❌' },
  manquant:   { label: 'Manquant',     labelEn: 'Missing',    color: '#6B6F76', bg: 'rgba(251,191,36,0.12)',  icon: '⚠️' },
  en_attente: { label: 'En attente',   labelEn: 'Pending',    color: '#6B6F76', bg: 'rgba(96,165,250,0.12)',  icon: '🕐' },
}
    ...
    <div style={{ minHeight: '100vh', background: '#FAFAF9', color: '#0E1116', fontFamily: 'system-ui,sans-serif' }}>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)', zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}
          ...
          <div style={{ background: '#FFFFFF', border: '1px solid #EBEBE9', borderRadius: 18, width: '100%', maxWidth: 440, overflow: 'hidden' }}
            ...
            <div style={{ padding: '20px 24px' }}>
              ...
              {/* Type */}
              <label style={{ fontSize: 12, fontWeight: 600, color: '#6B6F76', textTransform: 'uppercase', letterSpacing: 0.6, display: 'block', marginBottom: 6 }}>Type</label>
              ...
              {/* Statut */}
              <label style={{ fontSize: 12, fontWeight: 600, color: '#6B6F76', textTransform: 'uppercase', letterSpacing: 0.6, display: 'block', marginBottom: 8 }}>{lang === 'fr' ? 'Statut' : 'Status'}</label>
              ...
              {modal !== 'add' && form.fichier_url && !fichier && (
                <p style={{ fontSize: 12, color: '#3A3D40', marginBottom: 8 }}>
              ...
              <input
                ...
                style={{ width: '100%', padding: '9px 12px', background: '#F7F7F5', border: '1px solid #EBEBE9', borderRadius: 9, color: '#0E1116', fontSize: 13, marginBottom: 14, boxSizing: 'border-box', cursor: 'pointer' }}
              />

              {/* Notes */}
              <label style={{ fontSize: 12, fontWeight: 600, color: '#6B6F76', textTransform: 'uppercase', letterSpacing: 0.6, display: 'block', marginBottom: 6 }}>Notes</label>
              <textarea value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))}
                placeholder={lang === 'fr' ? 'Infos supplémentaires...' : 'Additional info...'}
                rows={2}
                style={{ width: '100%', padding: '10px 12px', background: '#F7F7F5', border: '1px solid #EBEBE9', borderRadius: 9, color: '#0E1116', fontSize: 14, marginBottom: 18, boxSizing: 'border-box', outline: 'none', resize: 'vertical', fontFamily: 'system-ui,sans-serif' }} />
      ...
      <main style={{ maxWidth: 780, margin: '0 auto', padding: '32px 20px 80px' }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 28, flexWrap: 'wrap' }}>
          {[
            { icon: '✅', val: nbValides,       label: lang === 'fr' ? 'valides' : 'valid',       color: '#3A3D40', bg: 'rgba(52,211,153,0.10)'   },
            { icon: '⚠️', val: nbExpirentBient, label: lang === 'fr' ? 'expirent bientôt' : 'expiring soon', color: '#6B6F76', bg: 'rgba(251,191,36,0.10)'  },
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', background: s.bg, border: `1px solid ${s.color}25`, borderRadius: 10 }}>
              ...
              <span style={{ fontSize: 13, color: '#6B6F76' }}>{s.label}</span>
            </div>
          ))}
        </div>
        ...
        {ready && suggestions.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            <p style={{ fontSize: 12, fontWeight: 600, color: '#6B6F76', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 10 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {suggestions.map(d => (
                <button key={d.type}
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', background: '#0E111610', border: `1px dashed ${'#0E1116'}40`, borderRadius: 20, color: '#3A3D40', fontSize: 13, fontWeight: 500, cursor: 'pointer' }}>
                  <span>{d.icone}</span>
        ...
        {!ready ? (
          ...
          <>
            <p style={{ fontSize: 13, fontWeight: 600, color: '#6B6F76', textTransform: 'uppercase', letterSpacing: 0.8, marginBottom: 14 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 32 }}>
              {docsRequis.map(requis => {
                ...
                return (
                  <div key={requis.type} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: '#FFFFFF', border: `1px solid ${borderColor}`, borderRadius: 12, transition: 'border-color 0.15s' }}>
                    <span style={{ fontSize: 22, flexShrink: 0 }}>{requis.icone}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
                        <p style={{ fontSize: 14, fontWeight: 600, color: '#0E1116' }}>{lang === 'fr' ? (doc?.nom || requis.nom) : (doc?.nom || requis.nom_en || requis.nom)}</p>
                        {requis.critique && !doc && (
                        ...
                        {expireSoon && !isExpired && (
                          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 20, background: 'rgba(251,191,36,0.15)', color: '#6B6F76', letterSpacing: 0.4 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                        ...
                        {doc?.notes && <span style={{ fontSize: 12, color: '#6B6F76' }}>· {doc.notes}</span>}
                      </div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                      {doc?.fichier_url && (
                        <button onClick={() => ouvrirDocument(doc.fichier_url)}
                          style={{ padding: '6px 12px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 8, color: '#3A3D40', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                          📄 {lang === 'fr' ? 'Voir' : 'View'}
                        </button>
                      )}
                      {doc ? (
                        <>
                          <button onClick={() => ouvrirEdition(doc)} style={{ padding: '6px 12px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 8, color: '#3A3D40', fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
                            ✏️
                          </button>
              ...
              <>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {docs.filter(d => !docsRequis.some(r => r.type === d.type)).map(doc => {
                    ...
                    return (
                      <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', background: '#FFFFFF', border: `1px solid ${alertExp ? '#6B6F7640' : '#EBEBE9'}`, borderRadius: 12 }}>
                        <span style={{ fontSize: 22 }}>📄</span>
                        <div style={{ flex: 1 }}>
                          <p style={{ fontSize: 14, fontWeight: 600, color: '#0E1116', marginBottom: 3 }}>{doc.nom}</p>
                        ...
                        <div style={{ display: 'flex', gap: 8 }}>
                          ...
                          <button onClick={() => ouvrirEdition(doc)} style={{ padding: '6px 12px', background: '#0E111612', border: `1px solid ${'#0E1116'}25`, borderRadius: 8, color: '#3A3D40', fontSize: 12, cursor: 'pointer' }}>✏️</button>
                          <button onClick={() => supprimer(doc)} disabled={deleting === doc.id}
            ...
            {docs.length === 0 && docsRequis.length === 0 && (
              <div style={{ textAlign: 'center', padding: '48px 24px', background: '#FFFFFF', border: '1px solid #EBEBE9', borderRadius: 16 }}>
                ...
                <p style={{ fontWeight: 600, fontSize: 16, color: '#0E1116', marginBottom: 8 }}>
