  ...
  return (
    <div style={{ minHeight: '100vh', background: '#FAFAF9', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ textAlign: 'center', maxWidth: 400 }}>
        {status === 'loading' && <p style={{ color: '#6B6F76' }}>{isFr ? 'Vérification...' : 'Verifying...'}</p>}
        {status === 'success' && (
          <>
            <div style={{ fontSize: 56, marginBottom: 20 }}>✅</div>
            <h1 style={{ fontSize: 24, fontWeight: 700, color: '#0E1116', marginBottom: 10 }}>
              {isFr ? 'Paiement confirmé' : 'Payment confirmed'}
            </h1>
            <p style={{ fontSize: 14, color: '#6B6F76', lineHeight: 1.7, marginBottom: 28 }}>
              {isFr
        ...
        {status === 'error' && (
          <>
            <div style={{ fontSize: 56, marginBottom: 20 }}>❌</div>
            <h1 style={{ fontSize: 22, fontWeight: 700, color: '#0E1116', marginBottom: 10 }}>
