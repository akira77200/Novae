        ...
        <div style={{ display: 'flex', gap: 4, marginBottom: 28, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 12, padding: 4 }}>
          {TABS.map(tab => (
            <button key={tab.id} onClick={() => setSection(tab.id)}
              style={{ flex: 1, padding: '9px 8px', borderRadius: 8, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', background: section === tab.id ? C.accent : 'transparent', color: section === tab.id ? '#fff' : C.muted }}>
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 24 }}>
              {[
                ...
                <div key={i} style={{ padding: '16px', background: C.surface, border: `1px solid ${c.color}30`, borderRadius: 14 }}>
                  ...
                  <p style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>{c.desc}</p>
                </div>
              ))}
            </div>
            ...
            <div style={{ overflowX: 'auto', marginBottom: 24 }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tbody>
                  {TABLEAU.map((row, i) => (
                    <tr key={i} style={{ background: row.highlight ? `${C.accent}04` : i % 2 === 0 ? C.surface : 'transparent' }}>
                      <td style={{ padding: '12px 14px', border: `1px solid ${C.border}`, fontWeight: 600, color: C.text }}>
        ...
        {section === 'quiz' && (
          <>
            {!quizTermine ? (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
                  <div style={{ flex: 1, height: 5, background: C.border, borderRadius: 3, overflow: 'hidden' }}>
                    <div style={{ width: `${(progression / QUESTIONS.length) * 100}%`, height: '100%', background: C.accent, borderRadius: 3, transition: 'width 0.3s' }} />
                  </div>
                  <span style={{ fontSize: 12, color: C.muted, fontWeight: 600, whiteSpace: 'nowrap' }}>{progression}/{QUESTIONS.length}</span>
                </div>
                ...
                {progression > 0 && (
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 20 }}>
                    {QUESTIONS.slice(0, progression).map((q, i) => (
                      <span key={i} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, background: `${C.accent}12`, color: C.accent2, border: `1px solid ${C.accent}20` }}>
                ...
                {QUESTIONS[qActuelle] && (() => {
                  ...
                  return (
                    <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '24px' }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: C.muted, textTransform: 'uppercase', letterSpacing: 0.7, marginBottom: 14 }}>
                        {lang === 'fr' ? `Question ${qActuelle + 1} sur ${QUESTIONS.length}` : `Question ${qActuelle + 1} of ${QUESTIONS.length}`}
                      </p>
                      <p style={{ fontSize: 17, fontWeight: 700, color: C.text, lineHeight: 1.5, marginBottom: 22 }}>
                        {q.q[lang] || q.q.fr}
                      </p>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                        {q.options.map((opt, i) => (
                          <button key={i} onClick={() => repondre(q.id, i, opt.score)}
                            style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '13px 16px', background: C.surface2, border: `1px solid ${C.border}`, borderRadius: 11, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}
                            ...
                            <div style={{ width: 30, height: 30, borderRadius: '50%', background: `${C.accent}12`, border: `1px solid ${C.accent}25`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: 13, color: C.accent2, flexShrink: 0 }}>
                              {String.fromCharCode(65 + i)}
                            </div>
                            <p style={{ fontSize: 14, color: C.text, lineHeight: 1.5 }}>{lang === 'fr' ? opt.fr : opt.en}</p>
                          </button>
                        ))}
                      </div>
                    </div>
              ...
              <div>
                {/* Résultat principal */}
                <div style={{ textAlign: 'center', padding: '32px 24px', background: C.surface, border: `1px solid ${C.border}`, borderRadius: 18, marginBottom: 24 }}>
                  <p style={{ fontSize: 48, marginBottom: 12 }}>
                    {resultat.recommande === 'univ' ? '🏛️' : resultat.recommande === 'college' ? '🔧' : '⚖️'}
                  </p>
                  <p style={{ fontSize: 22, fontWeight: 800, color: C.text, marginBottom: 8 }}>
                  ...
                  <p style={{ fontSize: 14, color: C.muted, lineHeight: 1.6, maxWidth: 500, margin: '0 auto 24px' }}>
                    {resultat.recommande === 'univ'
                  ...
                  <div style={{ display: 'flex', gap: 10, justifyContent: 'center', flexWrap: 'wrap' }}>
                    <button onClick={resetQuiz}
                      style={{ padding: '10px 20px', background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 9, color: C.muted, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                      ↺ {lang === 'fr' ? 'Recommencer' : 'Restart'}
                    </button>
                    <a href="/bourses"
                      style={{ padding: '10px 20px', background: C.accent, border: 'none', borderRadius: 9, color: '#fff', fontWeight: 600, fontSize: 13, textDecoration: 'none' }}>
                    ...
                    {(resultat.recommande === 'college' || resultat.recommande === 'egal') && (
                      <button onClick={() => setSection('passerelles')}
                        style={{ padding: '10px 20px', background: `${C.accent}15`, border: `1px solid ${C.accent}30`, borderRadius: 9, color: C.accent2, fontWeight: 600, fontSize: 13, cursor: 'pointer' }}>
                ...
                <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 14, padding: '18px 20px' }}>
                  <p style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {QUESTIONS.map(q => {
                      ...
                      return (
                        <div key={q.id} style={{ display: 'flex', gap: 10, padding: '9px 12px', background: C.surface2, borderRadius: 9 }}>
                          <span style={{ flexShrink: 0, fontSize: 14 }}>{fav}</span>
                          <div>
                            <p style={{ fontSize: 12, color: C.muted, marginBottom: 2 }}>{q.q[lang] || q.q.fr}</p>
                            <p style={{ fontSize: 13, color: C.text, fontWeight: 500 }}>{lang === 'fr' ? rep.fr : rep.en}</p>
                          </div>
                        </div>
                      )
                    })}
        ...
        {section === 'passerelles' && (
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {PASSERELLES.map((p, i) => (
                <div key={i} style={{ background: C.surface, border: `1px solid ${p.couleur}30`, borderRadius: 16, overflow: 'hidden' }}>
                  <div style={{ height: 4, background: p.couleur }} />
                  <div style={{ padding: '20px 22px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
                      <span style={{ fontSize: 28 }}>{p.emoji}</span>
                      <p style={{ fontSize: 16, fontWeight: 800, color: C.text }}>{p.titre[lang] || p.titre.fr}</p>
                    </div>
                    ...
                    {/* Exemples */}
                    <p style={{ fontSize: 11, fontWeight: 700, color: C.muted, textTransform: 'uppercase', letterSpacing: 0.6, marginBottom: 8 }}>
