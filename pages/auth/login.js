...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (err) { setError(err.message.includes('Invalid') ? 'Email ou mot de passe incorrect.' : err.message); setLoading(false); return }
    ...
    <div style={{ minHeight: '100vh', background: C.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'system-ui,sans-serif' }}>
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 700, color: C.text, letterSpacing: -0.5, marginBottom: 6 }}>{t.login_title}</h1>
          <p style={{ fontSize: 14, color: C.muted }}>Bienvenue sur Novae</p>
        </div>
        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '28px 28px 24px' }}>
          ...
          <form onSubmit={handleLogin}>
            ...
            <input type="email" value={form.email} onChange={e => set('email', e.target.value)} required placeholder="ton@email.com" style={inp} />
            ...
            <div style={{ textAlign: 'right', marginTop: 8, marginBottom: 20 }}>
              <Link href="/auth/forgot" style={{ fontSize: 12, color: C.muted, textDecoration: 'none' }}>{t.login_forgot}</Link>
            </div>

            {error && (
              <div style={{ padding: '10px 14px', background: `${C.error}15`, border: `1px solid ${C.error}40`, borderRadius: 9, color: C.error, fontSize: 13, marginBottom: 16 }}>
                {error}
              </div>
            )}
          ...
          <p style={{ textAlign: 'center', fontSize: 13, color: C.muted, marginTop: 20 }}>
            {t.login_no_account}{' '}
            <Link href="/auth/register" style={{ color: C.accent2, fontWeight: 600, textDecoration: 'none' }}>{t.login_signup}</Link>
          </p>
        </div>
      </div>
    </div>
