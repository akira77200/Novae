        ...
        <div style={{ background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: '26px 26px 22px' }}>
          ...
          {step === 1 && (
            <>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 18 }}>
                ...
                <span style={{ fontSize: 12, color: C.muted }}>{lang === 'fr' ? 'ou' : 'or'}</span>
                <div style={{ flex: 1, height: 1, background: C.border }} />
              </div>
              ...
              {form.email && !emailOk && <p style={{ fontSize: 11, color: C.error, marginTop: 4 }}>{lang === 'fr' ? 'Email invalide' : 'Invalid email'}</p>}
              ...
              {form.password && !passOk && <p style={{ fontSize: 11, color: C.error, marginTop: 4 }}>{lang === 'fr' ? 'Minimum 8 caractères' : 'Minimum 8 characters'}</p>}
              ...
              {form.confirm_password && !matchOk && <p style={{ fontSize: 11, color: C.error, marginTop: 4 }}>{lang === 'fr' ? 'Les mots de passe ne correspondent pas' : 'Passwords do not match'}</p>}
          ...
          {step === 2 && (
            <>
              ...
              {error && (
                <div style={{ padding: '10px 14px', background: `${C.error}15`, border: `1px solid ${C.error}40`, borderRadius: 9, color: C.error, fontSize: 13, marginTop: 14 }}>
                  {error}
                </div>
              )}

              <div style={{ display: 'flex', gap: 8, marginTop: 22 }}>
                <button onClick={() => setStep(1)} style={{ flex: 1, padding: '12px', background: 'transparent', border: `1px solid ${C.border}`, borderRadius: 10, color: C.muted, fontSize: 15, cursor: 'pointer' }}>
                  ← {t.back}
                </button>
